...
namespace CHARACTERS
{
    public abstract class Character
    {
        ...
        public virtual void SetPosition(Vector2 position)
        {
            ...
            (Vector2 minAnchorTarget, Vector2 maxAnchorTarget) = ConvertUITargetPositionToRelaveCharacterAnchorTart(position);
        ...
        private IEnumerator MovingToPosition(Vector2 position, float speed, bool smooth)
        {
            ...
            while (root.anchorMin != minAnchorTarget || root.anchorMax != maxAnchorTarget)
            {
                root.anchorMin = smooth ?
                    ...
                    : Vector2.MoveTowards(root.anchorMin, minAnchorTarget, speed * Time.deltaTime * 0.35f); // 0.35f -> ralentissement

                root.anchorMax = root.anchorMin + padding;

                if (smooth && Vector2.Distance(root.anchorMin, minAnchorTarget) <= 0.001f) // 0.001 -> suffisament proche faire un epsilon pour le futur
                {
                    root.anchorMin = minAnchorTarget;
                    root.anchorMax = maxAnchorTarget;
                    break;  
        ...
        protected (Vector2, Vector2) ConvertUITargetPositionToRelaveCharacterAnchorTart(Vector2 position)
        {
            Vector2 padding = root.anchorMax - root.anchorMin;

            // normalize max lenght anchor
            float maxX = 1f - padding.x;
            float maxY = 1f - padding.y;

            Vector2 minAnchorTarget = new(maxX * position.x, maxY * position.y);
