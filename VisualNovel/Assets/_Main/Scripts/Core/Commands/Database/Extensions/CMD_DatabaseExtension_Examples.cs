...
namespace TESTING
{
    public class CMD_DatabaseExtension_Examples : CMD_DatabaseExtension
    {
        new public static void Extend(CommandDatabase database)
        {
            ...
            database.AddCommand("print_mp", new Action<string[]>(PrintLines)); // multi

            // Add lamda with no parameters
            database.AddCommand("lambda", new Action(() => { Debug.Log("Printing a default message to console for lambda command."); }));
            ...
            // Add Coroutime with no parameters
        ...
        private static IEnumerator LineProcess(string data)
        {
            if (int.TryParse(data, out int num))
            {
                for (int i = 1; i <= num; i++)
                {
                    Debug.Log($"LineProcess Running... [{i}]");
                    yield return new WaitForSeconds(1);
                }
            }
        }
        ...
        private static IEnumerator MultiLineProcess(string[] data)
        {
            ...
            {
                Debug.Log($"Line[]Process Running... [{line}]");
        ...
        private static IEnumerator MoveCharacter(string direction)
        {
            ...
            // Get the variables I need. This would be defiened somewhere else
            ...
            while (Mathf.Abs(targetX - currentX) > 0.1f)
            {
                ...
                character.position = new(currentX, character.position.y, character.position.z);
                yield return null;
            }
        }
    }
